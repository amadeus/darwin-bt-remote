...
with DeusKVM advertising on the Mac. Process exit releases its native handles.
Select the existing paired Mac/DeusKVM entry. This requests GATT discovery;
it does not pair, unpair, write HID reports, or install anything.
#>
[CmdletBinding()]
param(
    [string] $DeviceName = 'DeusKVM',
    [ValidateRange(0, 60)]
    [int] $HoldSeconds = 20
)
...
'@
    ...
    return ,([DeusKVMDiagnostics.CollectionBridge]::Copy($Collection))
}
...
if ($matchesByName.Count -eq 1) {
    ...
    Write-Host 'Select the paired Mac/DeusKVM entry:'
