...
internal sealed class Removal(Action<string> progress, bool showResult) : IRemovalSteps
{
    ...
    public Task FinishFiles()
    {
        ...
        using var source = Assembly.GetExecutingAssembly().GetManifestResourceStream("BTRemote.Companion.FinishRemoval.ps1")
...
internal sealed class RemovalForm : Form
{
    ...
    public RemovalForm(bool showResult = true)
    {
        ...
        Shown += async (_, _) =>
        {
            using var installation = new Mutex(false, "Global\\BTRemoteCompanionInstall");
            var acquired = false;
            try
            {
